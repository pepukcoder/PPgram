...
fn update_css_theme(theme: &Theme) {
...
pub fn ThemeToggler() -> impl IntoView {
    let set_maybe_theme = use_theme_setter();

    let on_click = move |_| {
        set_maybe_theme.update(|current_theme| {
            let new_theme = match current_theme {
                Some(Theme::Dark) => Theme::Light,
                _ => Theme::Dark,
            };
            *current_theme = Some(new_theme);
        });
    };
    view! {
        <div class="flex flex-col">
        <button class="flex items-column m-2 justify-center p-1 border border-sky-500 dark:border-sky-600 rounded-full" on:click=on_click>
            <svg class="hidden dark:inline w-6 h-6">
                <path fill-rule="evenodd" clip-rule="evenodd" d="M17.715 15.15A6.5 6.5 0 0 1 9 6.035C6.106 6.922 4 9.645 4 12.867c0 3.94 3.153 7.136 7.042 7.136 3.101 0 5.734-2.032 6.673-4.853Z" class="fill-sky-400/20 stroke-sky-500"></path>
                ...
                <path fill-rule="evenodd" clip-rule="evenodd" d="M17 3a1 1 0 0 1 1 1 2 2 0 0 0 2 2 1 1 0 1 1 0 2 2 2 0 0 0-2 2 1 1 0 1 1-2 0 2 2 0 0 0-2-2 1 1 0 1 1 0-2 2 2 0 0 0 2-2 1 1 0 0 1 1-1Z" class="fill-sky-400/20 stroke-sky-500"></path>
            </svg>
...
pub fn ThemeProvider(children: Children) -> impl IntoView {
    ...
    create_effect(move |_| {
        ...
        if let Some(theme) = maybe_got {
            update_css_theme(&theme);
        }
    });
